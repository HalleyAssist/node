...
void Session::RemoveStream(const Stream* stream) {
  ...
  switch (stream->origin()) {
    case Stream::Origin::CLIENT:
      if (is_server())
        DecrementStat(&SessionStats::streams_in_count);
      else
        DecrementStat(&SessionStats::streams_out_count);
      break;
    case Stream::Origin::SERVER:
      if (is_server())
        DecrementStat(&SessionStats::streams_out_count);
  ...
  switch (stream->direction()) {
    case Stream::Direction::BIDIRECTIONAL:
      DecrementStat(&SessionStats::bidi_stream_count);
      break;
    case Stream::Direction::UNIDIRECTIONAL:
      DecrementStat(&SessionStats::uni_stream_count);
      break;
  }
