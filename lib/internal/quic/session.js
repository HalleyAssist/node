...
function sessionDestroy(session, error) {
  ...
  if (error) {
    ...
    if (state.closed) state.closed.reject(error);
    return;
  }

  if (isPromisePending(session[kState].handshake.promise))
    session[kState].handshake.resolve();
  if (state.closed) state.closed.resolve();

  if (state.handshake) state.handshake.reject('Session destroyed');
  if (state.clienthello) state.clienthello.reject('Session destroyed');
  if (state.ocsp) state.ocsp.reject('Session destroyed');
}
